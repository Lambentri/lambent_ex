defmodule LambentEx.Phoenix.LiveEditable.Interface.Tailwind3 do
  ...
  def render(%{ple_type: "select", ple_mode: "focus", ple_data: nil} = assigns) do
    ~H"""
      <div>
        <form style="display: inline;" phx-submit={@ple_action} phx-blur={@ple_action} phx-target={@myself} phx-click-away={@ple_action} phx-value-target="myfield">
  ...
  def render(%{ple_type: "select", ple_mode: "focus"} = assigns) do
    ~H"""
      <div>
        <form style="display: inline;" phx-submit={@ple_action} phx-blur={@ple_action} phx-target={@myself} phx-click-away={@ple_action}>
          <%= select :ok, :ord, @ple_options, phx_change: "reorder", phx_target: @myself, selected: @ple_data %>
        </form>
      </div>
    """
  end
