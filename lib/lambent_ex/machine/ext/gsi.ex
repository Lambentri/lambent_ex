...
defmodule LambentEx.Machine.Ext.GSI do
  ...
  defp read(:dota_ward_purchase_cooldown, state),
    do: state[:gs] |> get_in(["map", "ward_purchase_cooldown"])
  ...
  defp read(:dota_player_kill_streak, state),
    do: state[:gs] |> get_in(["player", "kill_streak"]) || 0
  ...
  defp read(:dota_player_gold_unreliable, state),
    do: state[:gs] |> get_in(["player", "gold_unreliable"]) || 1

  defp read(:dota_player_gold_reliable, state),
    do: state[:gs] |> get_in(["player", "gold_reliable"]) || 1

  defp read(:dota_player_gold_from_creep_kills, state),
    do: state[:gs] |> get_in(["player", "gold_from_creep_kills"]) || 0

  defp read(:dota_player_gold_from_hero_kills, state),
    do: state[:gs] |> get_in(["player", "gold_from_hero_kills"]) || 0

  defp read(:dota_player_gold_from_income, state),
    do: state[:gs] |> get_in(["player", "gold_from_income"]) || 0

  defp read(:dota_player_gold_from_shared, state),
    do: state[:gs] |> get_in(["player", "gold_from_shared"]) || 0
  ...
  defp read(:dota_hero_has_debuff, state),
    do: state[:gs] |> get_in(["hero", "has_debuff"]) || false
  ...
  defp read(:dota_hero_magicimmune, state),
    do: state[:gs] |> get_in(["hero", "magicimmune"]) || false
  ...
  defp read(:dota_hero_attributes_level, state),
    do: state[:gs] |> get_in(["hero", "attributes_level"]) || 0

  defp read(:dota_hero_respawn_seconds, state),
    do: state[:gs] |> get_in(["hero", "respawn_seconds"]) || 0

  defp read(:dota_hero_buyback_money, state),
    do: state[:gs] |> get_in(["hero", "buyback_cost"]) || 0

  defp read(:dota_hero_buyback_seconds, state),
    do: state[:gs] |> get_in(["hero", "buyback_cooldown"]) || 0

  # talents
  defp read(:dota_hero_aghs_scepter, state),
    do: state[:gs] |> get_in(["hero", "aghanims_scepter"]) || false

  defp read(:dota_hero_aghs_shard, state),
    do: state[:gs] |> get_in(["hero", "aghanims_shard"]) || false
  ...
  defp read(:dota_hero_ability_charge_cooldown, state),
    do: state |> Map.get("charge_cooldown") || 0
    ...
    %{
      health_fill:
        generate_fill(
          state,
          {@green, @whitish},
          read(:dota_health, state),
          read(:dota_health_max, state)
        ),
      health_bright: [],
      mana_fill:
        generate_fill(
          state,
          {@blue, @whitish},
          read(:dota_mana, state),
          read(:dota_mana_max, state)
        ),
      mana_bright: [],
      money_reliable_fill:
        generate_fill(
          state,
          {@yellow, @whitish},
          read(:dota_player_gold_reliable, state),
          read(:dota_player_gold, state)
        ),
